  ...
  return (
    <div className={className}>
      ...
      <div className="flex justify-center mb-8">
        <div className="inline-flex items-center bg-gray-100 dark:bg-dark-surface rounded-lg p-1">
          <button
            onClick={() => setIsYearly(false)}
            className={`px-4 py-2 font-medium rounded-md transition-colors ${
              !isYearly
                ? 'bg-white dark:bg-dark-input text-light-text-primary dark:text-dark-text-primary shadow-sm'
          ...
          >
            Yearly
            <span className="ml-1.5 text-green-600 dark:text-green-400" style={{ fontSize: 'var(--text-small)' }}>Save 17%</span>
          </button>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`relative rounded-2xl border-4 p-6 ${
                plan.highlight
                  ...
                  : 'border-gray-50 dark:border-dark-border'
              } ${
                isCurrentPlan
            ...
            >
              ...
              <div className="text-center mb-6">
                ...
                {price !== null && price > 0 && (
                  <span className="text-light-text-secondary dark:text-dark-text-secondary" style={{ fontSize: 'var(--text-body)' }}>
                    /{isYearly ? 'year' : 'month'}
                  </span>
                )}
              </div>
              ...
              <ul className="space-y-3 mb-6">
                {plan.features.map((feature, index) => (
                  <li key={index} className="flex items-start">
                    ...
                    <span className={`${feature.included ? 'text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-muted dark:text-dark-text-muted'}`} style={{ fontSize: 'var(--text-body)' }}>
                      {feature.text}
                    </span>
                  </li>
                ))}
